...
// for the fed during a single event
	std::string filename;
					...
					for (auto const ch : lay.second) {
						for (auto const roc : ch.second) {
							...
							for (auto const pix : roc.second) {
								...
								if (ch.first < 17) {
									glibpix[0].write((char*)&addressBuffer, 4);
									buffers[3].push_back(addressBuffer);
								}
								if ((ch.first <33) && (ch.first > 16)) {
									glibpix[1].write((char*)&addressBuffer, 4);
									buffers[4].push_back(addressBuffer);
								}
								if (ch.first > 32) {
									glibpix[2].write((char*)&addressBuffer, 4);
									buffers[5].push_back(addressBuffer);
								}
							}
						}
...
// the real main function
	...
	//std::cout<<"\n\nMaking histograms...\n";
	//makeHistograms(pStore); seg faulting
